...
export default function Register() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false);
      toast.success('Conta criada com sucesso! Vamos configurar sua empresa.');
      navigate('/onboarding');
    }, 1500);
  };
    ...
    <div className="min-h-screen bg-[#F8F9FF] flex flex-col items-center justify-center p-4">
      ...
      <Card className="w-full max-w-[400px] border-[#E4E4E7] border shadow-none bg-white rounded-xl overflow-hidden">
        <CardHeader className="text-center space-y-2 pt-8">
          <CardTitle className="text-2xl font-semibold tracking-tight text-[#09090B]">Criar nova conta</CardTitle>
          <CardDescription className="text-sm font-medium text-[#71717A]">Comece agora mesmo a automatizar seu atendimento</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 pt-4 px-8 pb-8">
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-xs font-bold text-[#71717A] uppercase tracking-wider">Nome Completo</Label>
              <Input id="name" placeholder="Como podemos te chamar?" required className="h-10 border-[#E4E4E7] rounded-lg focus:ring-1 focus:ring-[#6366F1]" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email" className="text-xs font-bold text-[#71717A] uppercase tracking-wider">E-mail Profissional</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="text-xs font-bold text-[#71717A] uppercase tracking-wider">Crie sua senha</Label>
              <Input id="password" type="password" placeholder="Mínimo 8 caracteres" required className="h-10 border-[#E4E4E7] rounded-lg focus:ring-1 focus:ring-[#6366F1]" />
            </div>
          ...
          <p className="text-[10px] text-center text-[#A1A1AA] mt-4 font-medium leading-relaxed">
            Ao clicar em criar conta, você concorda com nossos <Link to="#" className="underline">Termos</Link> e <Link to="#" className="underline">Privacidade</Link>.
          </p>
        </CardContent>
        <CardFooter className="flex justify-center border-t border-[#F4F4F5] bg-[#F8F9FF]/50 py-5">
          <p className="text-sm text-[#71717A] font-medium">
            Já tem uma conta? <Link to="/login" className="text-[#6366F1] font-bold hover:text-[#4F46E5] ml-1">Fazer login</Link>
          </p>
        </CardFooter>
      </Card>
